...
export interface MerchantIdeaInput {
  idea: string;
  audience: string;
...
export interface DynamicProductMatch {
  product: ProductIdea;
  score: ProductScore;
  matchScore: number;
  matchReasons: string[];
...
export interface DynamicSelectionResult {
  normalizedIdea: string;
  suggestedCategory: ProductCategory;
  confidence: '高' | '中' | '待验证';
  matches: DynamicProductMatch[];
  nextActions: string[];
  warnings: string[];
}
...
const categoryKeywordRules: Array<{ category: ProductCategory; keywords: string[] }> = [
  { category: 'ai-gadget', keywords: ['ai', 'AI', '智能', '语音', '机器人', '互动', 'ongo', 'Ongo', '宠物机器人', '故事机'] },
  { category: 'lighting', keywords: ['灯', '照明', '台灯', '小夜灯', '氛围灯', '灯串', '庭院灯'] },
  { category: 'hardware', keywords: ['五金', '挂钩', '门吸', '锁', '合页', '螺丝', '拉手'] },
  { category: 'small-appliance', keywords: ['小家电', '风扇', '电器', '充电', '厨房小电', '加湿', '电机'] },
  { category: 'storage', keywords: ['收纳', '置物', '整理', '盒', '篮', '挂袋'] },
  { category: 'apparel', keywords: ['服装', '衣服', 't恤', 'T恤', '防晒衣', '内衣', '袜'] },
  { category: 'pet', keywords: ['宠物', '猫', '狗', '粘毛', '猫砂'] },
  { category: 'kitchen', keywords: ['厨房', '油壶', '定时器', '密封', '饭盒'] },
  { category: 'cleaning', keywords: ['清洁', '抹布', '刷', '拖把', '洗护'] },
  { category: 'phone-accessory', keywords: ['手机', '支架', '数据线', '保护壳', '理线'] },
  { category: 'stationery', keywords: ['文具', '笔', '便签', '办公', '学生'] },
  { category: 'beauty-tools', keywords: ['美妆', '化妆刷', '粉扑', '睫毛', '美容'] },
  { category: 'personal-care', keywords: ['个护', '旅行', '分装瓶', '洗漱', '一次性毛巾'] },
  { category: 'baby', keywords: ['母婴', '宝宝', '儿童', '防撞', '安全锁'] },
  { category: 'outdoor', keywords: ['户外', '露营', '地钉', '营地', '野餐'] },
  { category: 'auto', keywords: ['车', '汽车', '车载', '号码牌', '内饰'] },
  { category: 'packaging', keywords: ['包装', '快递袋', '标签', '胶带', '纸箱'] },
  { category: 'party', keywords: ['节日', '派对', '气球', '装饰', '圣诞'] },
  { category: 'sports', keywords: ['运动', '健身', '跳绳', '握力', '弹力带'] },
];

function normalizeIdeaText(input: MerchantIdeaInput): string {
  return `${input.idea} ${input.audience}`.trim();
}

function inferCategoryFromIdea(input: MerchantIdeaInput): { category: ProductCategory; confidence: DynamicSelectionResult['confidence'] } {
  const text = normalizeIdeaText(input);
  const rankedCategories = categoryKeywordRules
    .map((rule) => ({
  ...
  if (!best || best.hits === 0) {
    return { category: 'storage', confidence: '待验证' };
  }

  return { category: best.category, confidence: best.hits >= 2 ? '高' : '中' };
}

function tokenizeIdea(text: string): string[] {
  return uniqueTake(
    text
      .split(/[\s,，.。/、;；:：|]+/)
...
function scoreProductAgainstIdea(
  product: ProductIdea,
  input: MerchantIdeaInput,
  suggestedCategory: ProductCategory,
): { score: number; reasons: string[] } {
  const ideaText = normalizeIdeaText(input);
  const tokens = tokenizeIdea(ideaText);
  const searchable = `${product.name} ${product.category} ${categoryLabels[product.category]} ${product.supplyBase} ${product.audience} ${product.pddQuery} ${product.procurementQuery}`;
  const keywordHits = tokens.filter((token) => searchable.includes(token));
  const categoryMatched = product.category === suggestedCategory;
  const contacts = selectOfflineContactsForDisplay(product.offlineContacts);
  const hasNearbyContact = contacts.some((contact) => contact.isNearby);
  const hasRemoteContact = contacts.some((contact) => !contact.isNearby);
  const priceFit =
    input.priceBand === 'low'
      ? product.sampleCostMax <= 800
      : input.priceBand === 'mid'
        ? product.sampleCostMax <= 1600
        : product.marginPotential >= 4;

  const score =
    keywordHits.length * 18 +
    (categoryMatched ? 36 : 0) +
    (input.preferNearby && hasNearbyContact ? 12 : 0) +
    (!input.acceptsRemote && hasRemoteContact && !hasNearbyContact ? -18 : 0) +
    (priceFit ? 10 : -4) +
    scoreProduct(product, input.mode).score * 0.35;

  const reasons = uniqueTake(
    [
      categoryMatched ? `匹配到${categoryLabels[product.category]}品类` : '',
      keywordHits.length > 0 ? `命中关键词：${keywordHits.slice(0, 3).join('、')}` : '',
      hasNearbyContact ? '有中山周边线下采样点' : '主要依赖外地扩展货源',
      priceFit ? '样品预算与输入价格带基本匹配' : '样品预算可能偏高，建议先少量拿样',
      product.afterSalesRisk >= 4 ? '售后风险偏高，必须先压测' : '售后风险可控，适合小批测款',
    ],
    4,
  );
...
export function buildDynamicSelection(input: MerchantIdeaInput): DynamicSelectionResult {
  const normalizedIdea = normalizeIdeaText(input);
  const { category: suggestedCategory, confidence } = inferCategoryFromIdea(input);
  const rankedMatches = productIdeas
    .map((product) => {
      const match = scoreProductAgainstIdea(product, input, suggestedCategory);
      return {
        product,
        score: scoreProduct(product, input.mode),
        matchScore: Math.round(match.score),
        matchReasons: match.reasons,
        dynamicTitle: `${input.idea.trim() || product.name} · ${product.name}`,
        dynamicSummary: `${categoryLabels[product.category]}方向：${product.procurementHint}`,
      };
    })
    .filter((match) => match.matchScore > 20 || match.product.category === suggestedCategory)
    .sort((a, b) => b.matchScore - a.matchScore)
    .slice(0, 4);

  return {
    normalizedIdea: normalizedIdea || '未输入具体想法',
    suggestedCategory,
    confidence,
    matches: rankedMatches,
    nextActions: [
      `先按“${categoryLabels[suggestedCategory]}”方向看 2-4 个候选，不直接大批备货。`,
      input.preferNearby ? '优先电话/地图复核中山附近拿样点，再安排半天线下看样。' : '可同时对比中山与外地货源，先看样品稳定性。',
      '展开商品卡片，逐项核对真实图、线下点、验货建议和避坑信号。',
    ],
    warnings: [
      '当前为规则匹配的动态方案，不代表已完成实时电话核验。',
      input.acceptsRemote ? '外地货源可纳入比价，但首批仍要控制样品数量。' : '如果附近货源不足，结果可能偏少，需要补充外地扩展货源。',
    ],
  };
}
